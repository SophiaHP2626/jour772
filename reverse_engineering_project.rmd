...
## Load homicide data
```{r}
homicides_wapo <- read_csv("~/Documents/GitHub/jour772/reverse_engineering_project/data-homicides/homicide-data.csv")
View(homicide_data)
```
...
```{r}
# Chunk 4: Combine all cities' data for comparison
combined_homicide_data <- bind_rows(
  num_hom_and_arrest_bmore_2014_2017,
  num_hom_and_arrest_chicago_2014_2017,
  num_hom_and_arrest_boston_2014_2017,
  num_hom_and_arrest_st_louis_2014_2017
) %>%
  mutate(city = case_when(
    year %in% num_hom_and_arrest_bmore_2014_2017$year ~ "Baltimore",
    year %in% num_hom_and_arrest_chicago_2014_2017$year ~ "Chicago",
    year %in% num_hom_and_arrest_boston_2014_2017$year ~ "Boston",
    year %in% num_hom_and_arrest_st_louis_2014_2017$year ~ "St. Louis"
  )) %>%
...
bmore_neighborhoods <- st_read("Community_Statistical_Areas_(CSAs)_Reference_Boundaries.geojson") %>%
clean_names()
