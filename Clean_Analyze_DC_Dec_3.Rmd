...
---
#### SOPHIA HERNANDEZ-PINA COMMENTS: 
#I was able to complete tasks 2-4 for my assigned work but I got stuck on question one (New graphic on the number of death cases by year, stacked bar chart with Cheltenham and Jessup). I was able to make the graph however it was congested and I could not figure out how to change that. Please reach out if you would like me to go back and fix anything over the weekend while you are going over everything. 
...
##SH: two different James Evans and one of them has an error on death certificate (death date is the same as birth date). The James Evans in question is supposed to be 11 years old however on the death certificate it says that he died May xx, 1917 which was also the written date of birth exactly. 
...
```
#SH: I deleted all the hashtags and ran the code, this is what I assumed we should do. 
mark duplicates
```{r}
...
dc <- dc |> 
  mutate(cause_death_update = paste(cause_of_death, sep = "; ")) |> 
...
```{r}
##SH: IGNORE THIS. THIS IS ME TROUBLESHOOTING.
dc |>
  ggplot(aes(x = death_year, y = cause_of_death, fill = place_of_death_county)) +
  geom_col(position = "stack", alpha = 0.85) +
  scale_fill_manual(values = c("Cheltenham" = "#E41A1C", "Jessup" = "#377EB8"),
                    labels = c("Cheltenham", "Jessup")) +
  scale_x_continuous(breaks = seq(1880, 1940, by = 10),
                     limits = c(1875, 1945),
                     expand = expansion(mult = 0.02)) +
  labs(
    title = "Total Death Cases by Year: Cheltenham vs Jessup",
    subtitle = "All causes of death combined, 1880-1941",
...
ggsave("deaths_by_location.png", width = 12, height = 6, dpi = 300)
```
...
##SH: I tried changing category_broad to cause_of_death, and it worked for 225-258 however it did not work for 259-269.
...
dc<-dc %>% 
  mutate(
  cat_respiratory = str_detect(cause_of_death, "(?i)tuberculosis|pneumonia|asthma|influenza|asphyxia|Suffocation|lung_disease"),
  )
  mutate(cause_of_death = case_when(
    str_detect(cause_of_death, "tuberculosis|pneumonia|asthma|influenza|asphyxia|Suffocation|lung_disease") ~ "respiratory",
    str_detect(cause_of_death, "meningitis|peritonitis|encephalitis|Diptheria|malaria") ~ "infection",
    str_detect(cause_of_death, "typhoid|gastric_ulcer") ~ "intenstinal",
    str_detect(cause_of_death, "Chronic Nephritis|acute parenchymatous nephritis anasarca") ~ "kidney disease",
    str_detect(cause_of_death, "cardiac_issues") ~ "cardiac",
    str_detect(cause_of_death, "hemorrhage|Compression of the Brain") ~ "trauma",
    str_detect(cause_of_death, "exhaustion|delirium|asthenia|Accidental Drowning|Prematurely 6 weeks gestation|Immaturely 6 month baby| Spinal Paralysis|No further information given") ~ "other",
     str_detect(cause_of_death, "exhaustion|asthenia") ~ "exhaustion",
    str_detect(cause_of_death, "no_details") ~ "no details",
    )) 
```
