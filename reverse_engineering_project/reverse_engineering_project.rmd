...
416-422: this combines the 2015-2017 homicide rate dat from all four cities, Baltimore, Chicago, Boston and St. Louis, into a single dataset for comparative analysis. The bind_rows() function stacks the four separate datasets on top of each other after adding a label for each city using mutate(). Then we used arrange() to sort the combined data first by city name in alphabetical orger and then by year per city, creating one master dataset that allowed us to compare homicide rates, arrest rates and population trends accros all of these cities post-2015.
