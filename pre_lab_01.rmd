...
**Answer**
There are 79366 observations of 9 variables in the UMD courses dataset.
### Task 6: Glimpse data
...
The "id" column is a charater (<chr>) data type. The "seats" column is a number <dbl> data set.
...
**Answer**
The first number group of the lines is giving the year and the month. the first line says 201712 which represents the year 2017 and the month 12, or December. the second part is showing how many classes are available for the term beginning in that month. In the same line mentioned above, there are 487 classes available during that term. 
...
The arrangement is different in this codeblock, the one above (Task 9). Task 9 ran oldest to newest. However in Task 10, by inputing the code arrangea(desc(count_classes)) the orrientation changed to newest to oldest. 
...
Compared to the previous codeblock, how did the output 
**Answer**
The new codeblack has more options than the previous two did, going based off term and department. The code changed to arrange(term, department). 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**
In this codeblock the code used (arrange(desc(total_seats))), showed the term, the number of classes and the seats available in all the classes during that term. 
...
In this code block, it now includes the mean and median of the amount of seats available each term. The top terms have the mean and median seats around the same in common. 
...
**Answer**
In this codeblock, They now provided certain departments which changes the number of courses and seats available in each column. 
### Task 12: Questions
...
1.  How can I quickly search how the total ammount of courses available throughout all of the terms provided. 

2.  How can I change between two departments when searching for courses. seats, etc.?

3.  How was I to properly answer these questions if I did not answer them correctly. 
