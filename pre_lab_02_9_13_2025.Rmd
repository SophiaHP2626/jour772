...
**Answer**
Adding the variable selected_journalism_courses we have now simplified our data set to focus on two columns, id and title.
...
**Answer:**
now instead of being <chr> it has changed to <date> by adding |> mutate(expenditure_date=mdy(expenditure_date)) which changed the format. 
...
**Answer**
In the new month column we are now looking at a specific year, the month, and a floor date which is the first day of every month. 
### Task 10: Mutate
...
**Answer**
error: code will not run and no error notification comes up.
...
**Answer**
Error: code will not run and no error notification will come up. 
...
**Answer**
I feel like this answer definitely provides more information but it seems like there is a lot more information, and we can narrow it down by individual candidates and then their votes in all of the counties to find the information for one specific person more simply.
...
**Answer**
ChatGPT Question: In Rstudio, what are two tidyverse functions that are similar to str_to_upper
ChatGPT Answer: str_to_lower() will convert all of the letters in the string to lowercase and str_to_title() will capitalize the first letter of each of the words in a string. 
...
**Answer**
no output provided, code will not run. 
```{r}
...
**Answer**
This answer makes sense. It is organizing all of the total expenses of each individual state.
...
1.  how do I mutate the code to give me one governor candidate only and his or her votes from each county. 

2.  How can I find what the expenses were for per state?

3.  how can I rearrange the order of expenses from smallest to largest?
