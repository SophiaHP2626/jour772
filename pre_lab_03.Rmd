...
"Vendor Name" is a character <chr> data type, the "Amount" field is a number <dbl> data type and "Date" is another character <chr> data type. 
```{r}
glimpse(payments)
```
...
There are 553 zip codes that do not have five characters, 
```{r}
payments |>
  group_by(`Vendor Zip`) |>
...
Fiscal Year <bdl> is showing every year there were grants and loans. The category <chr> holds the grant and loan titles and the count <int> shows how many grants and loans occurred that year. There are multiple N/As throughout the tibble, which shows missing data. The fiscal year of 2009 is also missing a loan section. There are also years with significantly more grants or loans than others. 
```{r}
md_grants_loans |> 
...
The data shows the zip code, a fical year, the amount in grants during that year, the description of what the grants are, the fiscal period and the date it was released. It is odd that there are two grants at the very top that are less than $100, however that needs to be researched more to prove its validity. 
```{r}
md_grants_loans |> 
  arrange(Amount)
```
...
It now provides who gave the grant and who recieved it. Everything else has remained the same from the previous lines of code and the years and amounts are not organized.
```{r}
# cleaning function
...
cleaned_md_grants_loans <- md_grants_loans is changing the name of the data from md_grants_loans to cleaned_md_grants_loans.
clean_names() removes special characters,
rename (source = grantor) changes the name of the column of grantor to source.
mutate(source = str_to_upper(source), grantee = str_to_upper(grantee), description = str_to_upper(description)) changes the columns to uppercase.
cleaned_md_grants_loans shows the cleaned up data set. 
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
There are 58 dupilcates possible.
```{r}
cleaned_md_grants_loans |>
  get_dupes()
```
...
The new dataframe has 19,453 rows.
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
The zip codes are split in half with the first five being put in the first zip code column and the rest gets put with the zip5 column. 
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
21202 is recieved $648,069 from the Maryland Tourism Board, and the zip code belongs to Baltimore city. It does make sense to me that it is the top recipient as it is one of the most popular spots within the state of maryland for tourists as it is the largest city in the state. 
```{r}
cleaned_md_grants_loans |> 
